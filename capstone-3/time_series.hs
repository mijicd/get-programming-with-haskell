...
instance Semigroup (TS a) where
    (<>) (TS [] []) ts2 = ts2
    (<>) ts1 (TS [] []) = ts1
    (<>) (TS t1 v1) (TS t2 v2) = TS times values
        where bothTimes = mconcat [t1, t2]
              times = [minimum bothTimes .. maximum bothTimes]
              original = foldl insertMaybePair Map.empty (zip t1 v1)
              updated = foldl insertMaybePair original (zip t2 v2)
              values = map (`Map.lookup` updated) times
...
makeTSCompare f = g
    where g (t1, Nothing) (t2, Nothing) = (t1, Nothing)
          g (_, Nothing) (t, v) = (t, v)
          g (t, v) (_, Nothing) = (t, v)
...
diffTS (TS times values) = TS times (Nothing : diffs)
    where shifted = tail values
          diffs = zipWith diffPair shifted values

meanMaybe :: Real a => [Maybe a] -> Maybe Double
meanMaybe vals = if all (== Nothing) vals then Nothing else Just avg
...
movingAverageTS (TS times values) n = TS times smoothed
    where ma = movingAvg values n
          nothings = replicate (n `div` 2) Nothing
          smoothed = mconcat [nothings, ma, nothings]
